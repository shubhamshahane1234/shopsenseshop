...
const Checkout = ({ cart, subTotal }: any) => {
  ...
  const initiatePayment = async () => {
    ...
    let txnRes = await a.json(); // get txnToken from your server side api

    let txnToken = txnRes.body.txnToken;
    console.log(txnToken, "pretransaction");
    var config = {
      root: "",
      flow: "DEFAULT",
      data: {
    ...
    console.log("typeof token:", typeof config.data.token);
    console.log("token value:", config.data.token);
    ...
    <div className="h-screen grid grid-cols-3">
      ...
      <div className="lg:col-span-2 col-span-3 bg-indigo-50 space-y-8 px-12">
        ...
        <button
          ...
          className="submit-button px-4 py-3 rounded-full  bg-[#2c9499] hover:bg-[#61a3a6]  text-white focus:ring focus:outline-none w-full text-xl font-semibold transition-colors"
        >
          Pay ₹{subTotal}
        </button>
      </div>
